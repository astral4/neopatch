//! neopatch_th14: latency reductions, optimizations, and other fixes for Touhou 14.
//!
//! Shipped as `dinput8.dll` next to `th14.exe`.

#[cfg(all(not(panic = "abort"), not(test), not(doc)))]
compile_error!("neopatch_th14 requires `panic = \"abort\"`");
...
use crate::config::{self as th14_config, Th14Config};
...
unsafe fn install_hooks() {
    unsafe {
        ...
        let (th14_cfg, core_cfg): (Th14Config, CoreConfig) = exe_dir
            .and_then(|d| read(d.join("neopatch.ini")).ok())
            .map_or_else(
                || (Th14Config::default(), CoreConfig::default()),
                |b| th14_config::parse(&core_config::decode_text(&b)),
            );
        drop(core_config::CONFIG.set(core_cfg));
        drop(config::CONFIG.set(th14_cfg));
        let core_cfg = core_config::CONFIG.get().unwrap();
        let th14_cfg = config::CONFIG.get().unwrap();
        ...
        log::init(&install_dir, core_cfg, host_exe_path.as_deref(), |w| {
            th14_config::write_manifest_extras(w, th14_cfg)
        });
        ...
        // Important: IAT patches should operate on th14.exe's import table, not ours!
        ...
        window::install(
            host_exe,
            &core_cfg.window,
            th14_cfg.resolution.dimensions(),
        ...
        // th14's input writer at `fcn.004018b0` does not read `DIJOYSTATE` offsets
