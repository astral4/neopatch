...
/// Parses INI text into a `(Th14Config, CoreConfig)` pair,
/// with defaults for any keys/sections the text omits.
pub(crate) fn parse(text: &str) -> (Th14Config, CoreConfig) {
    (parse_th14_only(text), config::parse_core_only(text))
}

fn parse_th14_only(text: &str) -> Th14Config {
    let mut cfg = Th14Config::default();
...
/// Writes th14-specific manifest lines that aren't already covered by the core configuration.
pub(crate) fn write_manifest_extras<W: Write + ?Sized>(
    w: &mut W,
    th14: &Th14Config,
) -> std::io::Result<()> {
    writeln!(w, "display.resolution={}", th14.resolution)
}
...
mod tests {
    ...
    fn default_matches_documented_defaults() {
        ...
        assert_eq!(th14.resolution, Resolution::R1280x960);
