...
/// OILP labels this site "Sleep NOP" but the bytes are the jump, not the call.
///
/// "frame limiter unconditional skip": flips a post-FPU `jne` to `jmp` so the loop
/// always lands past the deadline check. Together with the above,
/// this disengages the game's pacer in favor of ours.
///
