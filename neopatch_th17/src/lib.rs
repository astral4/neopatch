//! neopatch_th17: latency reductions, optimizations, and other fixes for Touhou 17.
//!
//! Shipped as `dinput8.dll` next to `th17.exe`.

#[cfg(all(not(panic = "abort"), not(test), not(doc)))]
compile_error!("neopatch_th17 requires `panic = \"abort\"`");
...
use crate::config::{self as th17_config, Th17Config};
...
unsafe fn install_hooks() {
    unsafe {
        ...
        let (th17_cfg, core_cfg): (Th17Config, CoreConfig) = exe_dir
            .and_then(|d| read(d.join("neopatch.ini")).ok())
            .map_or_else(
                || (Th17Config::default(), CoreConfig::default()),
                |b| th17_config::parse(&core_config::decode_text(&b)),
            );
        drop(core_config::CONFIG.set(core_cfg));
        drop(config::CONFIG.set(th17_cfg));
        let core_cfg = core_config::CONFIG.get().unwrap();
        let th17_cfg = config::CONFIG.get().unwrap();
        ...
        log::init(&install_dir, core_cfg, host_exe_path.as_deref(), |w| {
            th17_config::write_manifest_extras(w, th17_cfg)
        });
        ...
        // Important: IAT patches operate on th17.exe's import table, not ours.
        ...
        window::install(
            ...
            window::WindowPolicy::Restyle {
                framebuffer: th17_cfg.resolution.dimensions(),
        ...
        // th17's input writer at `fcn.00401690` does not read `DIJOYSTATE` offsets
