...
pub(crate) fn parse_config(text: &str) -> (Th17Config, CoreConfig) {
    (parse_th17_only(text), parse_core_only(text))
}
...
mod tests {
    ...
    fn parse_applies_known_keys() {
        let text = "
            ...
            resolution = 960x720
        ";
        let (th17, core) = parse_config(text);
