...
#[derive(Default)]
pub(crate) struct Th17Config {
    pub resolution: Resolution,
}

pub(crate) static CONFIG: OnceLock<Th17Config> = OnceLock::new();
...
/// Parses INI text into a `(Th17Config, CoreConfig)` pair,
/// with defaults for any keys/sections the text omits.
pub(crate) fn parse(text: &str) -> (Th17Config, CoreConfig) {
    (parse_th17_only(text), config::parse_core_only(text))
}

fn parse_th17_only(text: &str) -> Th17Config {
    let mut cfg = Th17Config::default();
...
/// Writes th17-specific manifest lines that aren't already covered by the core configuration.
pub(crate) fn write_manifest_extras<W: Write + ?Sized>(
    w: &mut W,
    th17: &Th17Config,
) -> IoResult<()> {
    writeln!(w, "display.resolution={}", th17.resolution)
}
...
mod tests {
    ...
    fn default_matches_documented_defaults() {
        ...
        assert_eq!(th17.resolution, Resolution::R1280x960);
    ...
    fn parse_applies_known_keys() {
        ...
        assert_eq!(th17.resolution, Resolution::R960x720);
    }
