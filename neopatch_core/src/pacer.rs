...
/// HR-timer wake-up safety margin in 100 ns units (i.e. this is 0.2 ms). The timer wakes the process this far before the deadline.
/// The final stretch is closed by a QPC spin so we land exactly on the deadline regardless of timer jitter.
const SAFETY_MARGIN_100NS: i64 = 2_000;

/// Wall-clock threshold beyond which `wait()` resyncs instead of running no-sleep catch-up. Below this gap,
/// missed deadlines are absorbed by skipping the sleep on the next few frames. Above it, the deadline is reseeded.
/// We go by wall-clock time rather than a period multiple so the meaning of "absorb hitches up to 50 ms"
/// stays consistent across `game_fps` values. There isn't anything special about 50 ms in particular.
const RESYNC_THRESHOLD_MS: i64 = 50;

pub static PACER: OnceLock<Pacer> = OnceLock::new();

/// Distinguishes when the game reads real-time input (lead applies) from when the game drives its own schedule (lead doesn't apply).
...
pub struct Pacer {
    ...
    // Created eagerly in `new`. When null, `sleep_until` falls through to Sleep+spin.
...
impl Pacer {
    ...
    pub fn new(policy: PacingPolicy) -> Self {
        // `QueryPerformanceFrequency` is documented as fixed at boot, so a single read suffices.
    ...
    /// Applies the given policy. A `target_fps` of 0 in the policy disables pacing.
    /// Also resets the deadline so the next `wait()` resyncs. Otherwise, a stale deadline
    ...
    /// Blocks until the next frame's deadline, then advances the deadline. This should be called once per `Present`.
    pub(crate) fn wait(&self, tok: &MainToken) {
        ...
        if deadline == 0 || now > deadline + self.resync_threshold_qpc {
            // First call or beyond the resync threshold. Below the threshold, the fallthrough path absorbs the gap via no-sleep catch-up.
            deadline = now + period;
            self.deadline_qpc.set(tok, deadline);
            return;
        }
        // We shift only the wait target, not the stored deadline. The long-run cadence still locks to `target_fps`.
...
/// Tries `CREATE_WAITABLE_TIMER_HIGH_RESOLUTION` first, then falls back to a plain waitable timer, then returns null if both fail.
...
/// QPC ticks per frame at `target_fps`. Returns 0 for `target_fps == 0`, which disengages the pacer.
