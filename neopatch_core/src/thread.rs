...
//!
//! [`MAIN_TID`] records the render thread's TID. It is claimed by the first thread to construct a [`MainToken`], atomically,
//! and verified on every subsequent construction. `MainToken` is a ZST witness that the holding thread is the render thread.
//! It is `!Send + !Sync`, so rustc rejects any safe code that tries to move it or share it across threads.
//! [`MainCell<T>`] accessors take `&MainToken`, propagating the "render-thread only" requirement to every call site at the type level.
//!
//! Render thread identity is established at first-hook entry, not at `DllMain`. The thread running `DllMain`
//! and the thread running the render loop are usually the same, but they diverge when something (e.g. the thprac launcher)
//! injects neopatch via `CreateRemoteThread` into a `CREATE_SUSPENDED` process before resuming the real initial thread.
...
///
/// This type is `!Send + !Sync`. Combined with the atomic claim in the constructor, this means: if the constructor returns,
/// then every downstream cell access through the resulting token is on the same thread that first constructed one.
/// (This assumes the claiming thread is never destroyed with its TID recycled onto a fresh thread, which would satisfy the CAS check.
/// However, the render thread is alive for the whole process, so this doesn't arise.)
pub struct MainToken(PhantomData<*const ()>);

impl MainToken {
    /// Constructs a `MainToken`. The first call claims [`MAIN_TID`] atomically; subsequent off-thread calls abort the process.
