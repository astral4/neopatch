//! neopatch_th095: neopatch for Touhou 9.5.

#[cfg(all(not(panic = "abort"), not(test), not(doc)))]
compile_error!("neopatch_th095 requires `panic = \"abort\"`");
...
use neopatch_core::{
    ansi, console, crash, d3d8, dinput8_export, exit_hooks, gdi_caps, input, log, process,
...
unsafe fn install_hooks() {
    // Neither of th095's helper executables (`custom.exe`, `replayview.exe`) imports our dinput8 proxy,
    // so `th095.exe` is the only expected host. A host without D3D8 gets nothing installed.
