...
///
/// Hook 1 (`updatefast_wrapper`): call-site rewrite at `0x4458a7` so the first
...
const UPDATEFAST_CALL_SITE: usize = 0x0044_58a7;
const REAL_UPDATEFAST: usize = 0x0044_6420;
const BGM_HANDLE_SLOT: usize = 0x004c_9098;
const IO_HANDLE_SLOT: usize = 0x004c_909c;
/// Loader-control flag. `0` initial; `1` releases the post-load waits in BGM thread proc
/// and I/O thread proc; `2` is the only escape from the inner busy-wait in `fcn.00449140`
...
const UPDATEFAST_CALL_BYTES: [u8; 5] = [0xe8, 0x74, 0x0b, 0x00, 0x00];
...
pub(crate) unsafe fn install_loader_sync_hooks() {
    unsafe {
        patch_jmp(
            IO_ERROR_SPLICE,
            &[0x8b, 0x04, 0xb5, 0xb0, 0x36, 0x4a, 0x00],
