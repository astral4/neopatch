//! neopatch_th08: neopatch for Touhou 8.

#[cfg(all(not(panic = "abort"), not(test), not(doc)))]
compile_error!("neopatch_th08 requires `panic = \"abort\"`");
...
unsafe fn install_hooks() {
    // Neither of th08's helper executables (`custom.exe`, `replayview.exe`) imports our dinput8 proxy,
    // so `th08.exe` is the only expected host. A host without D3D8 gets nothing installed.
