//! neopatch_th18: latency reductions, optimizations, and other fixes for Touhou 18.
//!
//! Shipped as `dinput8.dll` next to `th18.exe`.

#[cfg(all(not(panic = "abort"), not(test), not(doc)))]
compile_error!("neopatch_th18 requires `panic = \"abort\"`");
...
use crate::config::{self as th18_config, Th18Config, Th18DisplayMode};
...
unsafe fn install_hooks() {
    unsafe {
        ...
        let (th18_cfg, core_cfg): (Th18Config, CoreConfig) = exe_dir
            .and_then(|d| read(d.join("neopatch.ini")).ok())
            .map_or_else(
                || (Th18Config::default(), CoreConfig::default()),
                |b| th18_config::parse(&core_config::decode_text(&b)),
            );
        drop(core_config::CONFIG.set(core_cfg));
        drop(config::CONFIG.set(th18_cfg));
        let core_cfg = core_config::CONFIG.get().unwrap();
        let th18_cfg = config::CONFIG.get().unwrap();
        ...
        log::init(&install_dir, core_cfg, host_exe_path.as_deref(), |w| {
            th18_config::write_manifest_extras(w, th18_cfg)
        });
        ...
        // Important: IAT patches operate on th18.exe's import table, not ours.
        ...
        window::install(
            host_exe,
            &core_cfg.window,
            match th18_cfg.display_mode {
                Th18DisplayMode::Borderless => window::WindowPolicy::DeferToGame,
                Th18DisplayMode::Windowed | Th18DisplayMode::Fullscreen => {
                    window::WindowPolicy::Restyle {
                        framebuffer: th18_cfg.resolution.dimensions(),
        ...
        // th18's input writer at `fcn.00401650` reads `DIJOYSTATE2` button bytes
        // only (`rgbButtons` at +0x30..0xaf), never `rgdwPOV[]` (+0x20..0x2f),
        // so the default of `dpad = true` is safe.
